Create a single horizontal sprite strip for the Codex app digital pet `tuxie-actions` in the state `eating`.
...
Identity lock:
- Do not redesign the pet. Only change pose/action for the `eating` animation.
...
Animation action: eating from a small food bowl.


State-specific requirements:
- Show Tuxie eating from a small simple bowl: head up, head dipping toward bowl, tiny bite, chewing, relaxed reset.
- The bowl must be small, opaque, text-free, hard-edged, and physically touching or tucked under Tuxie's mouth/paws in every frame.
- Keep the bowl inside the same frame slot as Tuxie; do not add loose food pellets, crumbs, steam, scent lines, labels, UI, scenery, floor patches, or shadows.
- Preserve Tuxie's face, markings, palette, and silhouette; only the head/paw/body pose and small bowl position should change.
