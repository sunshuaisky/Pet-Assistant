Create a single horizontal sprite strip for the Codex app digital pet `tuxie-actions` in the state `play-ball`.
...
Identity lock:
- Do not redesign the pet. Only change pose/action for the `play-ball` animation.
...
Animation action: playing with a small toy ball.


State-specific requirements:
- Show Tuxie playing with a small toy ball: noticing the ball, pawing it, leaning into it, playful pounce, settled reset.
- The ball must be small, opaque, hard-edged, text-free, and touching or partly overlapping Tuxie's paw/body in every frame.
- Keep the ball inside the same frame slot as Tuxie; do not draw loose balls, repeated balls, rolling trails, dust, shadows, impact marks, or motion lines.
- Keep the background outside the pet and ball perfectly flat chroma key with no darker key-colored patches.
