Create a single horizontal sprite strip for the Codex app digital pet `tuxie-actions` in the state `play-wand`.
...
Identity lock:
- Do not redesign the pet. Only change pose/action for the `play-wand` animation.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Black-and-white exotic shorthair-style cat with a round flat face, yellow-green eyes, pink nose, white muzzle and chest, black cap over ears and head, black saddle patches on the back, compact chibi body, tiny legs, gentle curious expression. Preserve Tuxie's exact identity from the existing pet..
...
Animation action: playing with a cat teaser wand.


State-specific requirements:
- Show Tuxie tracking and batting at a small cat teaser wand: curious glance, paw reaches up, paw taps the toy, relaxed reset.
- The wand may be a small thin stick with a tiny attached feather or puff, but it must be physically connected to the pet pose or held/touched by a paw in every frame.
- Keep the wand simple, text-free, hard-edged, and inside each frame slot; do not let it become a separate floating object crossing slots.
- Do not draw wave marks, motion arcs, speed lines, sparkles, symbols, or other detached effects around the wand or paw.
