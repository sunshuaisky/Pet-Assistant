...
assert.doesNotMatch(mainSource, /data-setting-action="reset-user-avatar"/);
assert.match(mainSource, /display-reset-button/);
assert.match(stylesSource, /#409EFF/);
assert.match(mainSource, /import defaultUserAvatar from "\.\/assets\/default-touxiang\.jpg";/);
assert.match(mainSource, /<img src="\$\{defaultUserAvatar\}" alt="默认用户头像" \/>/);
...
assert.doesNotMatch(mainSource, /data-chat-delete/);
assert.doesNotMatch(stylesSource, /chat-delete/);
assert.match(mainSource, /closest\?\.\("\[data-chat-select\]"\)/);
assert.match(mainSource, /deleteChatConversation\(chatItem\.dataset\.chatSelect\)/);
